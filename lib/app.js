...
agentClient.emitter.on('result', function (message, from, date) {
	...
	(0, _ki1r0y.lock)(filename, function (unlock) {
		try {
			var extractor = jsonMessage.extractor,
			...
			if (fs.existsSync(filename)) {
				...
				var existingData = new _dataForge2.default.readFileSync(filename).parseJSON().parseDates("timestamp").setIndex("timestamp").orderBy(function (row) {
					return row.timestamp;
				}).startAt(last).tail(count - 1).toArray();
